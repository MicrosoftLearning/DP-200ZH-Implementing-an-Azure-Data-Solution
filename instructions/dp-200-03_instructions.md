...
在本实验室课程结束时，学生将能够解释为什么 Azure Databricks 可用于帮助数据科学项目。学生将配置 Azure Databricks 实例，然后创建一个工作区，用于从 Data Lake Store Gen II 存储执行简单的数据准备任务。最后，学生将使用 Azure Databricks 执行转换。
...
1. 在注册应用程序屏幕中，提供 **DLAccess** 的 **名称**，并在 **重定向 URI（可选）** 版块下，确保已选择 **网页** 并针对应用价值输入 **https://adventure-works.com/exampleapp**。设置值后，选择 **注册**。将出现 DLAccess 屏幕。

1. 在 **DLAccess** 注册的应用程序屏幕中，复制 **应用程序 Id和** 和 **Tenant ID** 并将其粘贴到记事本中。

1. 在 **DLAccess** 注册的应用程序屏幕，单击 **证书和机密**，然后单击 **+ 新客户机密..**

1. 对 **DL 访问密钥** 进行 **说明**，并说明密钥的 **持续时间** 为 **1 年内**。当完成时，单击 **保存**。

    >**重要事项**：单击“保存”时，将显示该密钥。你只有一次机会复制此密钥值到记事本中
