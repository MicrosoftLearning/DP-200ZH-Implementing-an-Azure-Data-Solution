...
1. 从 **“资源组”** 屏幕中，单击 **“+ 添加”**，使用以下设置创建第一个资源组：
...
1. 在“创建资源组”屏幕中，单击 **“查看 + 创建”**。

1. 在“创建资源组”屏幕中，单击 **“创建”**。

> **注意**：创建资源组大约需要 30 秒。你可以检查通知区域以检查创建完成的时间。
...
1. 在“新建”屏幕中，单击 **“搜索市场”** 文本框，然后输入**存储帐户**。在显示的列表中单击 **“存储帐户 - blob、文件、表、队列”**。
...
1. 从 **“创建存储帐户”** 屏幕，使用以下设置创建第一个存储帐户：
    ...
    - 在实例详细信息下，指定以下设置：
        ...
        - **存储帐户名称**：**Warehousexx**，其中 **XX** 是你的姓名缩写。
...
1. 在 **“创建存储帐户”** 屏幕中，单击 **“查看 + 创建”**。

1. 确认 **“创建存储帐户”** 屏幕后，单击 **“创建”**。

   > **注意**：创建存储帐户大约需要 90 秒，同时根据你定义的设置配置磁盘和磁盘配置。

### 任务 3：在存储帐户中创建和配置容器。

1. 在 Azure 门户中，消息说明_你的部署已完成_，单击按钮**转到资源**。

1. 在 **awsastudxx** 屏幕中，其中 **XX** 是你的姓名缩写，在 **“服务”** 下单击 **“Blob”**。
...
1. 在 **“新建容器”** 屏幕中，单击 **“创建”**。
   ...
   > **注意**：立即创建容器，容器将出现在 **“awrgstudxx - 容器”** 屏幕的列表中。

1. 重复步骤 4 -5 以创建名为 **phonecalls**的容器，公共访问级别为 **“私人（无匿名访问）”**

1. 重复步骤 4 -5 以创建名为 **tweets**的容器，公共访问级别为 **“私人（无匿名访问）”** 你的屏幕应如下图所示：

    ![Azure 门户中的容器列表](Linked_Image_Files/M02-E02-T03-img02.png)

### 任务 4：将一些图形上载到存储帐户的 images 容器中。

1. 在 Azure 门户中的 **“awsastudxx  - 容器”** 屏幕中，单击列表中的 **images** 项目。

1. 在 **images** 屏幕中，单击 **“上传”** 按钮。
...
1. 关闭 **“awsastudxx - 容器”** 屏幕，在 Azure 门户中，导航到 **“主页”** 屏幕。 

   > **注意**：上传文件将大约需要 5 秒。完成后，它们将出现在上传 blobs 屏幕的列表中。
...
1. 在距离实验室最近的位置创建并配置名为 **awdlsstudxx** 的存储账户，作为 Data Lake Store Gen2 存储类型，其中 **xx** 是你的姓名缩写。

1. 在 awsastudxx 存储帐户中创建名为 **logs** 和 **data** 的容器。
...
1. 在“新建”屏幕中，单击 **“搜索市场”** 文本框，然后输入 **“存储”**。在显示的列表中点击**存储帐户**。

1. 在**存储帐户**边栏选项卡上，单击**创建**。

1. 从**创建存储账户**边栏选项卡，使用以下设置创建一个存储帐户：
...
1. 在“Data Lake Storage Gen2”下，单击 **“分层名称空间”** 下的 **“已启用”**。

    ![在 Azure 门户的“创建存储帐户”屏幕中定义“分层命名空间”设置](Linked_Image_Files/M02-E03-T01-img01.png)

1. 在 **“创建存储帐户”** 边栏选项卡中，单击 **“查看 + 创建”**。
...
1. 在 **awdlsstudxx** 屏幕，其中 **xx** 是你的姓名缩写，单击 **“容器”**。

1. 在 **“awsastudxx - 容器”** 屏幕中，在左上角单击 **“+ 容器”** 按钮。

1. 在 **“新建”** 屏幕中，使用以下名称创建两个容器：
...
1. 在 **“新建容器”** 屏幕，单击 **“创建”**。
   ...
   > **注意**：立即创建文件系统，文件系统将出现在 **“awdlsstudxx - 容器”** 屏幕的列表中，如下所示。

    ![Azure 门户中列出的文件系统](Linked_Image_Files/M02-E03-T02-img01.png)

> **结果**：完成本练习后，你已创建名为 awdlsstudxx 的 Data Lake Gen2 的存储帐户，该帐户具有名为“数据”的文件系统。
...
1. 将一些数据文件上传到 Data Lake Gen II 存储帐户的容器。

### 任务 1：安装存储资源管理器

1. 在 Azure 门户中，在 **“awdlsstudxx - 容器”** 屏幕中，单击列表中的 **data** 项目。

1. 单击 **“概览”**，然后单击 **“在资源管理器中打开”**，然后单击 **“下载 Azure 存储资源管理器”** 超链接。

1. 你将进入以下网页 [Azure 存储资源管理器](https://azure.microsoft.com/zh-cn/features/storage-explorer/)，其中有一个 **“立即下载”** 按钮。单击该按钮。

1. 在 Microsoft Edge 对话框中单击**保存**，下载完成后，单击**查看下载**，在 Microsoft Edge 的下载屏幕中，单击**打开文件夹**。这将打开“下载”文件夹。

1. 双击该文件 **StorageExplorer.exe**，在“用户帐户控制”对话框中单击 **“是”**。

1. 在“许可协议”屏幕中，选择**我同意协议**附近的单选按钮，然后单击**安装**。

   > **注意**：存储资源管理器的安装大约需要 4 分钟。Azure 存储资源管理器允许你使用存储资源管理器轻松管理存储帐户的内容。上传、下载和管理 blob、文件、队列、表和 Cosmos DB 实体。它还使你可以轻松访问，以管理虚拟机磁盘。
...
### 任务 2：将数据文件上传到 Data Lake Gen II 存储帐户的 data 和 logs 容器。

1. 在 Azure 存储资源管理器中，单击箭头以展开你的订阅。

1. 在**存储帐户**下，搜索存储帐户**awdlsstudxx（ADLS Gen2）**，然后单击箭头将其展开。

1. 在 **Blob 容器**下，单击箭头将其展开并显示 **logs** 文件系统。单击 **log** 文件系统。
...
1. 重复上述步骤以从 **Labfiles\Starter\DP-200.2\logs** 文件夹将 preferences.JSON 文件上传到 Data Lake Store gen2 中的 **data** 文件系统

   > **注意**：上传文件将大约需要 5 秒。你将在 Azure 存储资源管理器中看到消息，其说明 **你的视图可能缺少数据。你想刷新吗？单击“是”**。完成后，所有两个文件将显示在上传 blob 屏幕的列表中。

    ![在 Azure 存储资源管理器中上传到容器的文件](Linked_Image_Files/M02-E04-T02-img02.png)

1. 在 Azure 存储资源管理器的数据文件系统中，单击 **“+ 新建文件夹”** 按钮。

1. 在“新建文件夹”屏幕的“新建文件夹”名称文本框中，键入 **“输出”**。
